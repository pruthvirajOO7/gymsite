 ...
 * Pipeline" section for the full step-by-step).
 *
 * What it does: the site's /api/booking serverless function (not the browser
 * directly) calls this webhook with a shared secret. This script verifies that
 * secret before writing anything, so the endpoint can't be used by anyone who
 * doesn't already know it — which in practice means only your own site.
 */
...
function doPost(e) {
  ...
  // Authentication: reject any request that doesn't include the shared secret
  // configured in this project's Script Properties (Project Settings -> Script
  // Properties). This is the actual access control on this endpoint — without
  // it, anyone who discovered this URL could write directly into the sheet.
  // Since this function is now only ever called server-side (from
  // api/booking.js), the URL and secret never reach the browser.
  const expectedSecret = PropertiesService.getScriptProperties().getProperty('BOOKING_SHARED_SECRET');
...
/**
 * One-time helper: paste your generated secret below (the same value you put
 * in Vercel's BOOKING_SHARED_SECRET environment variable), select
 * setBookingSecret from the function dropdown, and click Run once. Authorize
 * when prompted. This stores the secret in this project's Script Properties,
 * which doPost checks on every request.
 */
function setBookingSecret() {
  const secret = 'PASTE_THE_SAME_SECRET_YOU_PUT_IN_VERCEL_HERE';
  PropertiesService.getScriptProperties().setProperty('BOOKING_SHARED_SECRET', secret);
}
