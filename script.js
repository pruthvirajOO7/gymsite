...
// Trial booking form — submits to our own /api/booking serverless function,
// which then forwards (with a server-only shared secret) to a Google Apps
// Script webhook that logs the booking as a row in a Google Sheet. The Apps
// Script URL and secret live only in Vercel's environment variables — this
// file never contains them, so viewing page source reveals nothing useful.
// See README.md "Booking Pipeline" for setup steps.
...
if (trialForm) {
  trialForm.addEventListener('submit', (e) => {
    ...
    // Client-side validation is a UX nicety only — /api/booking and Apps
    // Script both re-validate everything independently. Never trust the client alone.
    ...
    // Honeypot tripped — pretend success so we don't tip off the bot, but don't send anything.
    if (honeypot) {
    ...
    fetch('/api/booking', {
      ...
      body: JSON.stringify({
        ...
        program,
        website: honeypot,
        ts: trialTsInput ? trialTsInput.value : ''
      })
    })
      .then(async (res) => {
        if (!res.ok) {
      ...
      .catch(() => {
        // Covers both "backend not deployed/configured yet" and genuine network errors —
        // we don't distinguish for the visitor, but do log a hint for you during setup.
        console.warn('Iron District: booking submission failed — check that /api/booking is deployed and GOOGLE_SHEET_WEBHOOK_URL / BOOKING_SHARED_SECRET are set in Vercel. See README.md "Booking Pipeline".');
