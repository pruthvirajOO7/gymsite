...
Free-session requests submitted through the "Visit" section are logged as rows in a Google Sheet, using a Google Apps Script web app as a free, serverless intermediary — no paid backend required.
...
4. **Initialize headers**: in the Apps Script editor, select `setupHeaderRow` from the function dropdown and click **Run**. Authorize the script when prompted (it runs only against your own sheet). This adds `Timestamp / Name / Email / Program` as column headers.
5. **Deploy as a web app**: **Deploy → New deployment → Web app**, with:
   - Execute as: **Me**
   - Who has access: **Anyone**

   Copy the resulting web app URL (`https://script.google.com/macros/s/.../exec`).
6. **Connect the site**: in `script.js`, set:
   ```js
   const GOOGLE_SHEET_WEBHOOK_URL = '<your web app URL>';
   ```

From that point forward, every booking submitted through the site appears as a new row in the sheet in real time.
...
**Booking webhook**
- **Honeypot field** — a hidden input real visitors never see; a filled value silently drops the submission.
- **Timing check** — submissions arriving faster than a human could plausibly type are silently dropped.
- **Server-side validation** — name/email length limits, email format checks, and a fixed allow-list for the `program` field, enforced in Apps Script itself rather than relying on client-side checks alone.
